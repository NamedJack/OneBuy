...
package wang.wongxd.wquicklib.custom.switchIconView;
...
//  ######################################## 以下是为了展示角标##########################
    ...
    private float redCircleSize = 15;
    private Paint paint;
    private RectF rectF;
    ...
    private void initProperty(Context context, AttributeSet attributeSet) {
        TypedArray typedArray = context.obtainStyledAttributes(attributeSet, R.styleable.SwitchIconView);
        redCircleSize = typedArray.getDimension(R.styleable.SwitchIconView_circleSize, redCircleSize);
        circleColor = typedArray.getColor(R.styleable.SwitchIconView_circleBgColor, Color.RED);
        textColor = typedArray.getColor(R.styleable.SwitchIconView_textColor, Color.WHITE);
        paint = new Paint();
